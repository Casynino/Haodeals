              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1 w-80 bg-background border border-white/18 z-50 shadow-2xl">
                  <div className="px-4 py-2.5 border-b border-white/10 flex items-center justify-between">
                    <p className="text-[12px] tracking-widest text-foreground/60">Notifications</p>
                    <span className="text-[12px] text-foreground/40">{notifications.length} total</span>
                  </div>
                  <div className="max-h-72 overflow-y-auto divide-y divide-white/6">
                    {notifications.length === 0 ? (
                      ...
                      <div key={n.id} className={`px-4 py-3 ${!n.read ? "bg-foreground/[0.03]" : ""}`}>
                        ...
                        <p className="text-[12px] text-foreground/50 mt-1">{n.body}</p>
                        <p className="text-[12px] text-foreground/35 mt-1">{new Date(n.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <DropdownMenuContent className="w-56 bg-background border border-white/15 p-0 font-mono text-xs" align="end">
                ...
                {!isAdmin && (
                  ...
                  >
                    ...
                    <span>Messages</span>
                    {msgUnreadCount > 0 && (
                      <span className="ml-auto w-4 h-4 flex items-center justify-center bg-green-400/80 text-black text-[11px] font-bold">
