    ...
    <div className="border border-yellow-400/18 bg-yellow-400/[0.025] px-4 py-3.5">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-foreground/50 leading-relaxed">{COPY[variant]}</p>
          <div className="flex items-center gap-4 mt-2 flex-wrap">
            {PERKS.map(({ Icon, label }) => (
              <div key={label} className="flex items-center gap-1 text-[10px] text-foreground/35">
        ...
        {/* CTA */}
        <button className="shrink-0 mt-0.5 text-[10px] tracking-widest text-yellow-400/65 border border-yellow-400/25 px-2.5 py-1.5 hover:bg-yellow-400/[0.07] transition-colors whitespace-nowrap">
          NOTIFY ME
        </button>
      </div>
    </div>
