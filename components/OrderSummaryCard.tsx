    ...
    <div className="border border-white/12 bg-foreground/[0.015] mb-5 overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 flex items-center gap-2 min-w-0 flex-wrap">
          <span className="text-[10px] tracking-[0.18em] text-foreground/65 font-mono">{ref}</span>
          <span className={`text-[8px] tracking-widest border px-2 py-0.5 flex items-center gap-1 shrink-0 ${statusMeta.badge}`}>
            <StatusIcon className="h-2.5 w-2.5" />
            {statusMeta.label.toUpperCase()}
          </span>
        </div>

        <span className="text-green-400/75 text-[11px] font-mono font-bold shrink-0 mr-1">
      ...
      {expanded && (
        <div className="border-t border-white/8">
            ...
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] text-foreground/75 leading-snug">{item.product.name}</p>
                ...
                {/* Line total */}
                <p className="text-[10px] text-green-400/65 font-mono font-medium shrink-0">
          ...
          <div className="border-t border-white/8 px-4 py-3 bg-foreground/[0.025]">
            <p className="text-[8px] tracking-[0.3em] text-foreground/25 mb-2">INVOICE SUMMARY</p>
            <div className="space-y-1 mb-2">
              {order.items.map((item) => (
                <div key={item.id} className="flex items-start justify-between gap-4 text-[9px]">
            ...
            <div className="flex items-center justify-between border-t border-white/8 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] text-green-400/60 tracking-wide">PAYMENT CONFIRMED</span>
              </div>
          ...
          <div className="border-t border-white/8 px-4 py-2.5 flex items-start gap-2">
            ...
            <div>
              <p className="text-[8px] tracking-[0.25em] text-foreground/25 mb-0.5">DELIVERY ADDRESS</p>
              <p className="text-[9px] text-foreground/55 leading-relaxed">{order.address}</p>
            </div>
          </div>
          ...
          {order.trackingEvents.length > 0 && (
            <div className="border-t border-white/8 px-4 py-3">
              <p className="text-[8px] tracking-[0.3em] text-foreground/25 mb-3">ORDER TIMELINE</p>
                    ...
                    <div key={evt.id} className="flex gap-3">
                      ...
                      <div className="pb-3 flex-1 min-w-0">
                        <div className="flex items-baseline gap-2 flex-wrap">
                          <span className={`text-[9px] tracking-wide font-medium ${isLast ? evtMeta.badge.split(" ")[0] : "text-foreground/40"}`}>
                            {evtMeta.label}
                          </span>
                          <span className="text-[8px] text-foreground/22">
                        ...
                        <p className="text-[9px] text-foreground/40 leading-relaxed mt-0.5">
                          {evt.message}
                        </p>
                      </div>
                    </div>
          ...
          <div className="border-t border-white/6 px-4 py-2 flex items-center justify-between bg-foreground/[0.01]">
            <span className="text-[8px] tracking-[0.25em] text-foreground/20">ORDER PLACED</span>
