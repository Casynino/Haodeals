...
import { ChevronLeft, Send, Package, User } from "lucide-react"
  ...
  if (!conversation) {
    return (
      <div className="container mx-auto px-4 py-16 text-center font-mono">
        ...
        <Link href="/admin/messages" className="text-[10px] text-foreground/30 hover:text-foreground/60 transition-colors mt-3 inline-block">
          ← Back to messages
        </Link>
      </div>
    )
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-3xl font-mono flex flex-col" style={{ minHeight: "calc(100vh - 80px)" }}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
              <span className="inline-flex items-center gap-1.5 text-[10px] text-foreground/50 border border-white/10 px-2 py-0.5">
              ...
              {conversation.order && (
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 text-[10px] text-foreground/45 hover:text-foreground/70 transition-colors border border-white/10 px-2 py-0.5 hover:border-white/25"
                >
                  <Package className="h-2.5 w-2.5" />
                  {conversation.order.trackingId
                    ? `#${conversation.order.trackingId}`
                    : `#${conversation.order.id.slice(0, 8).toUpperCase()}`}
                </Link>
              )}
            </div>
          </div>
          ...
          return (
            <div key={msg.id} className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] space-y-1 ${isAdmin ? "items-end" : "items-start"} flex flex-col`}>
                ...
                <div className={`flex items-center gap-1.5 text-[9px] text-foreground/30 ${isAdmin ? "flex-row-reverse" : ""}`}>
                  {isAdmin && (
      ...
      <form onSubmit={handleSend} className="border-t border-white/10 pt-4">
        ...
        <p className="text-[9px] text-foreground/25 mt-1.5">
